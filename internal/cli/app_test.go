...
func TestInstallGitHubGeneratesRepeatedFailureNotifications(t *testing.T) {
	dir := t.TempDir()
	path := writeConfig(t, dir, strings.ReplaceAll(twoProjectConfig(), "  - name: second\n    url: {env: SECOND_URL}\n    api_key: {env: SECOND_KEY}\n", ""))
	workflow := filepath.Join(dir, "workflow.yml")
	h := &harness{}
	args := []string{
		"--config", path, "install", "github", "--sb-heartbeat-version", "v0.2.0", "--output-path", workflow,
		"--workflow-config", "sb-heartbeat.yaml",
		"--github-notification-webhook-secret", "SB_HEARTBEAT_NOTIFICATION_WEBHOOK", "--notify-after", "4",
	}
	if code := cli.Execute(context.Background(), args, h.dependencies()); code != 0 {
		t.Fatalf("code=%d stderr=%q", code, h.stderr.String())
	}
	...
	if !strings.Contains(h.stdout.String(), "GitHub secret: SB_HEARTBEAT_NOTIFICATION_WEBHOOK") {
		t.Fatalf("notification binding guidance missing: %q", h.stdout.String())
	}
}

func TestInitGitHubGeneratesRepeatedFailureNotifications(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, "sb-heartbeat.yaml")
	workflow := filepath.Join(dir, "workflow.yml")
	h := &harness{}
	args := []string{
		...
		"--scheduler", "github", "--workflow-output", workflow, "--sb-heartbeat-version", "v0.2.0",
		"--workflow-config", "sb-heartbeat.yaml",
		"--github-notification-webhook-secret", "SB_HEARTBEAT_NOTIFICATION_WEBHOOK", "--notify-after", "2",
	}
	if code := cli.Execute(context.Background(), args, h.dependencies()); code != 0 {
		t.Fatalf("code=%d stderr=%q", code, h.stderr.String())
	}
	data, err := os.ReadFile(workflow)
	if err != nil || !strings.Contains(string(data), "--notify-after 2") {
		t.Fatalf("workflow=%q err=%v", data, err)
	}
}

func TestInitRejectsNotificationOptionsWithoutGitHubScheduler(t *testing.T) {
	for name, flags := range map[string][]string{
		"notification":       {"--github-notification-webhook-secret", "SB_HEARTBEAT_NOTIFICATION_WEBHOOK"},
		"empty notification": {"--github-notification-webhook-secret="},
		"threshold":          {"--notify-after", "2"},
		"annotations":        {"--github-annotations"},
		"artifact":           {"--github-artifact-retention-days", "7"},
		"workflow config":    {"--workflow-config", "config/sb-heartbeat.yaml"},
		"workflow output":    {"--workflow-output", ".github/workflows/custom.yml"},
		"runtime version":    {"--sb-heartbeat-version", "v0.2.0"},
	} {
		t.Run(name, func(t *testing.T) {
			dir := t.TempDir()
			configPath := filepath.Join(dir, "sb-heartbeat.yaml")
			h := &harness{}
			args := append([]string{"init", "--non-interactive", "--output-path", configPath, "--project-name", "demo"}, flags...)
			if code := cli.Execute(context.Background(), args, h.dependencies()); code != 2 {
				t.Fatalf("code=%d stderr=%q", code, h.stderr.String())
			}
...
func TestInstallGitHubRejectsIncompleteNotificationOptions(t *testing.T) {
	for name, flags := range map[string][]string{
		"threshold without secret": {"--notify-after", "2"},
		"explicit empty secret":    {"--github-notification-webhook-secret="},
		"explicit zero threshold":  {"--github-notification-webhook-secret", "WEBHOOK", "--notify-after", "0"},
	} {
		t.Run(name, func(t *testing.T) {
			dir := t.TempDir()
			path := writeConfig(t, dir, strings.ReplaceAll(twoProjectConfig(), "  - name: second\n    url: {env: SECOND_URL}\n    api_key: {env: SECOND_KEY}\n", ""))
...
func TestInitGitHubRejectsExplicitZeroNotificationThreshold(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, "sb-heartbeat.yaml")
	workflow := filepath.Join(dir, "workflow.yml")
	h := &harness{}
	args := []string{
		"init", "--non-interactive", "--output-path", configPath, "--project-name", "demo",
		"--scheduler", "github", "--workflow-output", workflow, "--sb-heartbeat-version", "v0.2.0",
		"--github-notification-webhook-secret", "WEBHOOK", "--notify-after", "0",
	}
	if code := cli.Execute(context.Background(), args, h.dependencies()); code != 2 {
		t.Fatalf("code=%d stderr=%q", code, h.stderr.String())
	}
